	...
	<main class="main">
        ...
        <div class="breadcrumbs">
            <div class="container">
                ...
                <span class="parent"> <a href="{{ route('library.index') }}"> Library </a> </span>

                <i class="fa fa-chevron-right"></i>

                <span class="child"> <a href="{{ route('university.library.book', ['slug' => $book->slug]) }}"> {{ $book->title }} </a> </span>

                <i class="fa fa-chevron-right"></i>

                <span class="child"> {{ __("Payment") }} </span>
            </div>
        </div>
        ...
        <section class="services-hood">
            ...
            <div class="row">
                ...
                <p class="text-center f-20 text-dark">  Amount :
                   <strong>{{ number_format($book->price) }} FCFA</strong>
                </p>
...
@section('scripts')
    ...
    function next()
    {
        window.location.href = "/library/mybooks";
    }
    ...
    $(document).ready(function(){
        ...
        $pay.click(function(){
            ...
            if(isValidNumber(number))
            {
                ...
                $.ajax({
                    url : '/api/subscribe/book',
                    method : 'post',
                    data : { _token:token, book:code, user:user, number:number, type:type },
                    error : function(e)
                    {
                        console.log(e);
                        stopLoading();
